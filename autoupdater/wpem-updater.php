...
class WPEM_Updater {
	...
	private function init_key_ui() {
		if ( ! $this->api_key ) {
			...
			add_filter( 'plugin_action_links_' . $this->plugin_name, array( $this, 'activation_links' ) );
			$this->add_notice( array( $this, 'key_notice' ) );
		} else {
			add_action( 'after_plugin_row_' . $this->plugin_name, array( $this, 'multisite_updates' ), 10, 2 );
			add_filter( 'plugin_action_links_' . $this->plugin_name, array( $this, 'deactivation_links' ) );
		}
	...
	public function activation_links( $links ) {
		$links[] = '<a href="' . add_query_arg( array('post_type' => 'event_listing', 'page' => 'license'), admin_url( 'edit.php' ) ) . '">' . __('Activate licence', 'wp-event-manager-registrations') . '</a>';
		return $links;
	}
	...
	public function deactivation_links( $links ) {
		$links[] = '<a href="' . remove_query_arg( array( 'deactivated_licence', 'activated_licence' ), add_query_arg( $this->plugin_slug . '_deactivate_licence', 1 ) ) . '">' . __('Deactivate licence', 'wp-event-manager-registrations') . '</a>';
		return $links;
	}
