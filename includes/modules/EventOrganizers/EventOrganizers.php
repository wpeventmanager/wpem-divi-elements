...
class WPEM_Event_Organizers extends ET_Builder_Module {
	...
	public function init() {
		$this->name = esc_html__( 'Event Organizers', 'wpem-wpem-event-dashboard' );
	}
	...
	public function render( $attrs, $content = null, $render_slug ) {
		...
		$shortcode = sprintf(
			'[event_organizers order="%1$s" orderby="%2$s" show_thumb="%3$s" show_count="%4$s"]',
			esc_attr( $order ),
			esc_attr( $orderby ),
			esc_attr( $show_thumb ),
			esc_attr( $show_count ),
